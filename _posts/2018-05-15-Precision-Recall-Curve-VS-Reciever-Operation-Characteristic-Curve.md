...
위 그림의 ROC와 PR 커브는 두개의 알고리즘을 학습시킨 결과로 그린 것이다. 학습 데이터는 negative 샘플에 편중된 데이터셋이었다.
...
2. ROC 공간에서 어떤 커브가 다른 커브보다 위에 있다면 (dominate) PR 공간에서도 위에 있다. 그리고 그 반대도 마찬가지다.
3. ROC 커브의 convex hull을 이용해 achievable PR curve를 그릴 수 있다.

위 세 가지에 대한 증명이 궁금하다면 직접 논문을 읽어보시라. 여기서는 각각이 무슨 의미가 있는지만 짚어보자.
우선 연관성 1은 ROC 커브를 PR 커브로 변환 할 수 있다는 얘기다. PR에서 ROC로도 가능하다. 리콜이 0인 경우는 매우 특이한 케이스를 얘기하는 것이니 크게 중요치 않다.
연관성 2에서 위에 있다 (dominate) 라는 얘기는 말그대로 모든 구간에서 어떤 커브가 다른 커브를 감싸고 있거나 최소 겹쳐있다는 얘기다.
그러면 연관성 2는 ROC 공간에서 위에 있는 커브는 PR 공간에서도 위에 있고, 반대로 PR 공간에서 위에 있는 커브는 ROC 공간에서도 위에 있다고 얘기하고 있다.
커브가 위에 있고 밑에 있고를 따지는 이유는 위에 있는 커브는 성능 좋은 분류기를 의미하기 때문이다.
연관성 3을 얘기하기 위해 ROC 공간에서 convex hull이 무엇을 의미하는지 보자.
...
그러면 ROC 공간의 convex hull을 PR 공간으로 변환하면 (연관성 1) PR 공간에서도 주어진 점으로 만들 수 있는 어떤 커브들 보다도 항상 위에 존재하는 (연관성 2) achievable PR curve를 얻게 된다 (그림 (c)).
