...
우선 1은 ROC 커브를 PR 커브로 변환 할 수 있다는 얘기다. PR에서 ROC로도 가능하다. 리콜이 0인 경우는 매우 특이한 케이스를 얘기하는 것이니 크게 상관없다.
...
그러면 ROC 공간의 convex hull을 PR 공간으로 변환하면 (1번) PR 공간에서도 주어진 점으로 만들 수 있는 어떤 커브들 보다도 항상 위에 존재하는 (2번) achievable PR curve를 얻게 된다.
